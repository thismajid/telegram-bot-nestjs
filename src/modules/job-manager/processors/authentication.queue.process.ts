...
export class AuthenticationQueueProcess {
  ...
  async default(job: Job<{ message: Message }>) {
    await this.jobManagerService.createNextProcess(
      job.data.message,
      undefined,
      {
        queue: QueuesEnum.Authentication,
        process:
          QueueProcessesEnum.AuthenticationSetFirstNameAndWaitForReceiveLastName,
      },
    );
  ...
  async setFirstNameAndWaitForReceiveLastName(job: Job<{ message: Message }>) {
    ...
    await this.jobManagerService.createNextProcess(
      ...
      { firstName },
      {
        queue: QueuesEnum.Authentication,
        process:
          QueueProcessesEnum.AuthenticationSetLastNameAndCompleteRegistration,
      },
    );
    await this.telegramBotService.sendMessage(
      job.data.message.chat.id,
  ...
  async setLastNameAndCompleteRegistration(
    job: Job<{ message: Message; data: Record<string, unknown> }>,
  ) {
    ...
    if (
      ...
      await this.telegramBotService.sendMessage(
        job.data.message.chat.id,
        '❌ ثبت نام شما با شکست مواجه شد؛ 🔃 لطفا مجددا امتحان کنید',
      );
